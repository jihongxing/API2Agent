...
- Go Control Plane Hosted Admin Trusted Gateway Service Dogfood Closeout + Phase Review v0 is complete:
  - the Control Plane-side hosted trusted-gateway path can pause.
  - remaining gateway, secret-rotation, permission-source, header-stripping, and tenant-partitioning risks are documented.
  - the next task is production gateway boundary design.
  - see `docs/en-US/GO_CONTROL_PLANE_HOSTED_ADMIN_TRUSTED_GATEWAY_SERVICE_DOGFOOD_CLOSEOUT_PHASE_REVIEW.md`.
- Go Control Plane Hosted Admin Trusted Gateway Production Boundary Design v0 is complete:
  - gateway-to-Control-Plane trust boundary is defined.
  - trusted header strip/rewrite rules and secret rotation policy are defined.
...
- Go Control Plane Hosted Admin Trusted Gateway Production Boundary Closeout + Phase Review v0 is complete:
  ...
  - the next task is a gateway contract harness design.
  - see `docs/en-US/GO_CONTROL_PLANE_HOSTED_ADMIN_TRUSTED_GATEWAY_PRODUCTION_BOUNDARY_CLOSEOUT_PHASE_REVIEW.md`.
- Go Control Plane Hosted Admin Gateway Contract Harness Design v0 is complete:
  - local gateway harness responsibilities are defined.
  - static public auth and identity policy are defined.
  - header stripping, trusted claim injection, and request/idempotency propagation are defined.
  - negative spoofing cases and dogfood evidence requirements are named.
  - see `docs/en-US/GO_CONTROL_PLANE_HOSTED_ADMIN_GATEWAY_CONTRACT_HARNESS_DESIGN.md`.
- Go Control Plane Hosted Admin Gateway Contract Harness Implementation v0 is complete:
  - local dogfood-only gateway harness is implemented with Python standard library HTTP serving.
...
- Agent Capability Compiler Quality Diagnostics v0 is complete:
  - generated packages now include additive `diagnostics.json`.
  - `api2agent diagnose` can print human-readable or JSON diagnostics.
...
- Agent Capability Compiler OpenAPI Security Requirement Combinations Closeout + Phase Review v0 is complete:
  - security requirement combinations are accepted as complete.
...
- Agent Capability Compiler OpenAPI Discriminator Handling Design v0 is complete:
...
- Agent Capability Compiler OpenAPI Discriminator Handling Closeout + Phase Review v0 is complete:
...
- Agent Capability Compiler OpenAPI Response Shape Documentation Closeout + Phase Review v0 is complete:
...
- Agent Capability Compiler OpenAPI Real-Spec Calibration Design v0 is complete:
...
- Agent Capability Compiler OpenAPI Summary Noise Reduction Closeout + Phase Review v0 is complete:
...
- Agent Capability Compiler OpenAPI Generic Example Reduction Design v0 is complete:
...
- Agent Capability Compiler OpenAPI Cached Real-Spec Corpus Expansion Implementation v0 is complete:
...
- Go Control Plane Hosted Admin Gateway Permission Source Closeout + Phase Review v0 is complete:
  - the static dogfood permission source is accepted as a v0 trust-boundary proof, not production authorization.
...
- Go Control Plane Tenant-Partitioned Registry Mutation Private Endpoint Design v0 is complete:
...
- Go Control Plane Tenant-Partitioned Registry Mutation Private Endpoint Live Dogfood + Closeout v0 is complete:
...
- Go Control Plane Hosted Permission Store Contract Harness v0 is complete:
  - the local hosted admin gateway harness now resolves public principals through a store-shaped subject/membership/role/grant read model before forwarding.
  - policy version, fingerprint, decision id, required permission evidence, gateway-local membership/revocation/stale-policy failures, and Control Plane second-gate behavior are covered.
  - the next hosted-readiness task is contract harness closeout and phase review.
  - see `docs/en-US/GO_CONTROL_PLANE_HOSTED_PERMISSION_STORE_CONTRACT_HARNESS_IMPLEMENTATION_REPORT.md`.
- Go Control Plane Hosted Permission Store Contract Harness Closeout + Phase Review v0 is complete:
  - the store-shaped lookup proof is accepted for v0.
...
- Go Control Plane Hosted Permission Store Schema Closeout + Phase Review v0 is complete:
  - the durable hosted permission-store schema boundary is accepted for v0.
...
- Go Control Plane Hosted Permission Store Read Model Live Postgres Dogfood v0 is complete:
  - a real local Postgres dogfood now applies the schema, seeds hosted permission rows, and calls the internal read model with pgx.
  - allowed admin evidence, missing membership, suspended membership, revoked/missing permission, no active policy, and ambiguous active policy are proven against real SQL.
...
- Go Control Plane Hosted Permission Decision Persistence Integrity Hardening Design v0 is complete:
  - the design specifies controlled evidence comparison, duplicate-equivalent no-op behavior, conflicting duplicate `PERMISSION_DECISION_INTEGRITY_CONFLICT`, caller failure semantics, sentinel constraints, metadata allowlist, retention/privacy stance, tests, and live dogfood probes.
  - public CRUD, OAuth/OIDC, invitation/session lifecycle, production gateway deployment, marketplace, vault, billing, workflow, automatic propagation, policy write APIs, and Data Plane mutable reads remain out of scope.
...
- Go Control Plane Hosted Permission Decision Persistence Integrity Hardening Closeout + Phase Review v0 is complete:
  - duplicate/conflicting decision integrity is accepted for local v0.
  - live Postgres dogfood proves equivalent duplicates no-op, conflicting duplicates fail closed, row counts stay stable, and private Control Plane audit is unchanged.
...
- Go Control Plane Hosted Permission Decision Retention Boundary Live Dogfood + Closeout v0 is complete:
  - live dogfood accepted the local retention/history boundary proof.
  - artifact evidence includes 16 redacted decision history rows, cross-project history count 0, support/operator audit count 6 -> 7, and legal-hold cleanup candidate proof.
  - hosted permission decision retention boundary lane completion is 100% for local v0.
  - broader Hosted Control Plane phase completion is estimated at 72%.
  - customer-facing decision history endpoints, public CRUD, OAuth/OIDC, invitation/session lifecycle, production gateway deployment, marketplace, vault, billing, workflow, automatic propagation, policy write APIs, and Data Plane mutable reads remain out of scope.
