...
- Go Control Plane Hosted Permission Decision Persistence Closeout + Phase Review v0 已完成：
  - local v0 persistence proof 被接受。
  - live Postgres dogfood 证明 15 条 secret-safe decision rows、gateway-local auth failures 不进入 persistence，以及 allowed-write-failure fail-closed behavior。
...
- Go Control Plane Hosted Permission Decision Persistence Production Boundary Design v0 已完成：
  - design 为 production-shaped v0 保持 decision persistence 由 gateway 拥有并同进程执行。
  - allowed decisions 必须在 forwarding 前同步持久化；不允许 forwarding 后静默 buffering。
  - bounded connection lifecycle、retry/backoff、conflict semantics、observability、retention/privacy、customer-visible history stance 和 schema hardening requirements 已文档化。
  - public CRUD、OAuth/OIDC、invitation/session lifecycle、production gateway deployment、marketplace、vault、billing、workflow、automatic propagation、policy write APIs 和 Data Plane mutable reads 仍然 out of scope。
  - 下一项 hosted-readiness task 是 hosted permission decision persistence production boundary implementation。
  - 详见 `docs/cn-ZH/GO_CONTROL_PLANE_HOSTED_PERMISSION_DECISION_PERSISTENCE_PRODUCTION_BOUNDARY_DESIGN.md`。
- Go Control Plane Hosted Permission Decision Persistence Production Boundary Implementation v0 已完成：
  - local gateway-owned writer 现在会为 hosted permission decisions 记录 canonical `evidence_fingerprint`。
  - schema 增加了 tenant/time、subject/time、retention 和 policy-fingerprint indexes，用于 production-shaped evidence queries。
  - persisted rows 会记录 bounded write timeout、transient retry budget、production boundary version 和 secret-safe metadata。
  - allowed persistence timeout 和 unavailable writes 会在 forwarding 前 fail closed；transient write failures 会在很小的 request budget 内 retry。
  - public CRUD、OAuth/OIDC、invitation/session lifecycle、production gateway deployment、marketplace、vault、billing、workflow、automatic propagation、policy write APIs 和 Data Plane mutable reads 仍然 out of scope。
...
- Go Control Plane Hosted Permission Decision Retention + Customer History Boundary Design v0 已完成：
  - design 将 hosted permission decision rows 的 default retention stance 设为 90 days。
  - history visibility classes 是 `hidden`、`tenant_visible_candidate` 和 `support_only`。
  - tenant/project scoped query semantics、redacted customer-visible fields、access-control expectations、support/operator audit、export、deletion、cleanup 和 legal-hold boundaries 已文档化。
  - customer-facing decision history endpoints 仍然 deferred。
  - public CRUD、OAuth/OIDC、invitation/session lifecycle、production gateway deployment、marketplace、vault、billing、workflow、automatic propagation、policy write APIs 和 Data Plane mutable reads 仍然 out of scope。
...
- Go Control Plane Hosted Permission Decision Retention Boundary Live Dogfood + Closeout v0 已完成：
  - live dogfood 接受了 local retention/history boundary proof。
  - artifact evidence 包含 16 条 redacted decision history rows、cross-project history count 0、support/operator audit count 6 -> 7，以及 legal-hold cleanup candidate proof。
  - hosted permission decision retention boundary lane completion 对 local v0 是 100%。
  - 更大的 Hosted Control Plane phase completion 估算为 72%。
  - customer-facing decision history endpoints、public CRUD、OAuth/OIDC、invitation/session lifecycle、production gateway deployment、marketplace、vault、billing、workflow、automatic propagation、policy write APIs 和 Data Plane mutable reads 仍然 out of scope。
...
- Go Control Plane Hosted Permission Policy Mutation Boundary Contract Harness v0 已完成：
  - local/private policy mutation helpers 现在覆盖 draft、validation、review、promotion、rollback、idempotency、audit 和 conflict behavior。
  - promotion 会更新 gateway read-model-compatible decision evidence。
  - rollback 创建新的 active version，并移除 promoted grant access，不重写 historical decision evidence。
  - idempotency replay/conflict、stale-base conflict、scope violation 和 duplicate grant conflict 已有测试覆盖。
  - in-process dogfood report helper 证明 promotion、rollback、replay、conflict、gateway decision changes、audit count 和 secret-safe evidence。
  - public CRUD、OAuth/OIDC、invitation/session lifecycle、production gateway deployment、customer-facing decision history、marketplace、vault、billing、workflow、automatic propagation、policy write APIs 和 Data Plane mutable reads 仍然 out of scope。
